#!/usr/bin/env python3
"""Upload batch 481-500 to tmpfiles.org (fallback file.io / 0x0.st)."""
...
files = sorted(OUT_DIR.glob("48[1-9]*.jpg")) + sorted(OUT_DIR.glob("49[0-9]*.jpg")) + sorted(OUT_DIR.glob("500_*.jpg"))
...
def upload_tmpfiles(path, retries=2):
    ...
    req = urllib.request.Request("https://tmpfiles.org/api/v1/upload", data=body, headers={"Content-Type": f"multipart/form-data; boundary={boundary}"})
    for attempt in range(retries):
        try:
            ...
            if url:
                return url + "?download=1" if "?" not in url else url, None
...
def upload_fileio(path, retries=2):
    with open(path, "rb") as f:
        data = f.read()
    req = urllib.request.Request("https://file.io", data=data, headers={"Content-Type": "image/jpeg", "Content-Disposition": f'attachment; filename="{path.name}"'})
    for attempt in range(retries):
        try:
            with urllib.request.urlopen(req, context=ctx, timeout=120) as r:
                d = json.loads(r.read().decode())
...
Path("/root/alonda/scripts/batch_481_500_uploads.json").write_text(json.dumps(results, indent=2, ensure_ascii=False))
