#!/usr/bin/env python3
"""Regen script for batch 1195-1214.
- Slot 1200: 'Isis' triggered sensitive-prompt filter (status 1026). Replaced with Hathor-themed variant.
- Slots 1201-1214: not yet generated. Re-generate with same prompts as gen_batch_1195_1214.py.
"""
...
# Slot 1200 — replaced 'Isis' (sensitive) with 'Hathor' (Egyptian goddess of love/music, NOT sensitive)
SHOTS = [
    ('1200_hathor_ancient_egypt',
     'Egyptian goddess Hathor with cow-horned solar crown and outstretched protective wings, holding a sistrum and an ankh, '
...
def main():
    ...
    for i, (key, scene) in enumerate(SHOTS, 1):
        ...
        time.sleep(2)
    results_path = Path(chr(47) + _R + chr(47) + 'alonda' + chr(47) + 'scripts') / ('batch_regen_' + str(START) + '_' + str(START + len(SHOTS) - 1) + '_results.json')
